...
const IndexersTable = () => {
  const [state, setState] = useState({
    ...initialState,
    ...SORT_STATE,
    sortDirection: SORT_STATE.sortDirection === 'asc' ? 0 : 1,
  });
  ...
  const toggleDirection = () => {
    ...
    updateSort(state.sortField, DIRECTIONS[sortDirection]);
  };
  ...
  const { loading: loadingIndexers, indexers, refetch } = GetIndexers(
    PAGINATION_STATE.skip,
    PAGINATION_STATE.first,
    SORT_STATE.sortField,
    state.sortDirection === 0 ? 'asc' : 'desc'
  );
  const loading = {
  ...
  const updateSort = (orderBy: string, orderDirection: string) =>
    refetch({
      skip: PAGINATION_STATE.skip,
      first: PAGINATION_STATE.first,
      orderBy,
      orderDirection,
    });

  const body = loadingIndexers ? (
    <TmDataLoading src='' {...loading} />
  ) : indexers !== null ? (
    indexers.map((validator, i) => (
      <LiValidator key={validator.id} index={i} validator={validator} />
    ))
  ) : (
