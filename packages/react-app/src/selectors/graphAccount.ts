...
import {
  ...
  GraphAccount,
  GraphAccountView,
} from '../types/theGraphProtocol';
import { convertToChecksum } from '../utils/helpers';

const formatDelegatorTotals = (
  totals: DelegatorTotals | null
): DelegatorTotalsView | null => {
  return totals != null
    ? {
        ...totals,
        totalStakedTokensDisplay: nativeToDisplayBalance(
          totals.totalStakedTokens
        ),
        totalUnstakedTokensDisplay: nativeToDisplayBalance(
          totals.totalUnstakedTokens
        ),
        totalRealizedRewardsDisplay: nativeToDisplayBalance(
          totals.totalRealizedRewards
        ),
      }
    : null;
};

const formatCuratorTotals = (
  totals: CuratorTotals | null
): CuratorTotalsView | null => {
  return totals !== null
    ? {
        ...totals,
        totalSignalledTokensDisplay: nativeToDisplayBalance(
          totals.totalSignalledTokens
        ),
        totalUnsignalledTokensDisplay: nativeToDisplayBalance(
          totals.totalUnsignalledTokens
        ),
        totalWithdrawnTokensDisplay: nativeToDisplayBalance(
          totals.totalWithdrawnTokens
        ),
        realizedRewardsDisplay: nativeToDisplayBalance(totals.realizedRewards),
        annualizedReturnDisplay: nativeToDisplayBalance(
          totals.annualizedReturn
        ),
